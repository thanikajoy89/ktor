...
internal class SelectorHelper {
    ...
    private val closeQueue = LockFreeMPSCQueue<Long>()
    ...
    fun notifyClosed(descriptor: Long) {
    ...
    private fun selectionLoop() {
        ...
        try {
            ...
            val closeSet = mutableSetOf<Long>()

            var wakeupDescriptor = wakeupSignal.wakeupDescriptor()

            while (!interestQueue.isClosed) {
                val event = EventInfo(
                    wakeupDescriptor.convert(),
                    SelectInterest.READ,
                    Continuation(EmptyCoroutineContext) { }
                )
                watchSet.add(event)
                var maxDescriptor = fillHandlers(watchSet, readSet, writeSet, errorSet)
                if (maxDescriptor == 0L) continue

                maxDescriptor = max(maxDescriptor + 1, wakeupDescriptor + 1)

                try {
                    selector_pselect(maxDescriptor.toInt() + 1, readSet, writeSet, errorSet).check()
                } catch (_: PosixException.BadFileDescriptorException) {
                    // Thrown if the descriptor was closed.
                }

                val wakeUpTriggered = processSelectedEvents(
                    watchSet,
                    closeSet,
                    completed,
                    readSet,
                ...
                if (wakeUpTriggered) {
                    wakeupDescriptor = wakeupSignal.wakeupDescriptor()
                }
            }
    ...
    private fun addInterest(
        ...
        select_fd_add(event.descriptor.convert(), set)
        select_fd_add(event.descriptor.convert(), errorSet)

        check(select_fd_isset(event.descriptor.convert(), set) != 0)
        check(select_fd_isset(event.descriptor.convert(), errorSet) != 0)
    }

    @OptIn(ExperimentalForeignApi::class)
    private fun processSelectedEvents(
        watchSet: MutableSet<EventInfo>,
        closeSet: MutableSet<Long>,
        completed: MutableSet<EventInfo>,
        readSet: CValue<selection_set>,
        writeSet: CValue<selection_set>,
        errorSet: CValue<selection_set>,
        wakeupDescriptor: Long?
    ): Boolean {
        var result = false
        while (true) {
        ...
        for (event in watchSet) {
            ...
            if (select_fd_isset(event.descriptor.toInt(), errorSet) != 0) {
                completed.add(event)
                event.fail(IOException("Fail to select descriptor ${event.descriptor} for ${event.interest}"))
                continue
            }

            if (select_fd_isset(event.descriptor.toInt(), set) == 0) continue

            if (event.descriptor == wakeupDescriptor) {
                result = true
                wakeupSignal.check()
                continue
            }
