...
internal actual class SignalPoint : Closeable {
    ...
    init {
        val (read, write) = memScoped {
            ...
            repeat(2) { index ->
                nonBlocking(pipeDescriptors[index].convert())
            }
    ...
    actual fun wakeupDescriptor(): Long = readDescriptor.toLong()
    ...
    actual fun signal() {
        synchronized(lock) {
            ...
            memScoped {
                ...
                val result = write(writeDescriptor.convert(), array, 1.convert())
                if (result < 0) return
    ...
    override fun close() {
        synchronized(lock) {
            ...
            close(writeDescriptor.convert())
            readFromPipe()
            close(readDescriptor.convert())
        }
    }
    ...
    private fun readFromPipe(): Int {
        ...
        memScoped {
            ...
            do {
                val result = read(readDescriptor.convert(), buffer, 1024.convert()).convert<Int>()
                if (result < 0) {
